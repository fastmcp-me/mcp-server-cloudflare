...
if (cmd === 'init') {
  ...
  if (rest.length > 0) {
    throw new Error(`Usage: npx @gutmutcode/mcp-server-cloudflare init [account_id]`)
  }
  ...
  if (!accountId && !config.accountId) {
    throw new Error(`Missing account ID. Usage: npx @gutmutcode/mcp-server-cloudflare run [account_id]`)
  }
  if (rest.length > 0) {
    throw new Error(`Too many arguments. Usage: npx @gutmutcode/mcp-server-cloudflare run [account_id]`)
  }
